...
class Entorno3SAT(ParallelEnv):
    metadata = {"render_modes": ["human"], "name": "voto_3sat_v8_mappo_comunitario_libre"}

    # Actor ve presion de demanda para votar en variables neutras
    LOCAL_OBS_DIM = 32    
    GLOBAL_STATE_DIM = 42 
    TOTAL_OBS_DIM = 74   

    def __init__(self, num_agentes=40, num_variables=10, max_pasos=1):
    ...
    def reset(self, seed=None, options=None):
        ...
        # Presion neta y magnitud para el Actor
        self.presion_neta = (self.demanda_favor - self.demanda_contra) / self.num_agentes
        self.presion_magnitud = (self.demanda_favor + self.demanda_contra) / self.num_agentes
    ...
    def step(self, actions):
        ...
        for agent_id, accion in actions.items():
            self.papeletas[agent_id] = accion
        ...
        for agent in self.agents:
            ...
            if any(resultado_leyes[var_idx] == deseo for var_idx, deseo in clausula):
                clausulas_satisfechas += 1

        premio_global = float(clausulas_satisfechas)
        rewards = {agent: premio_global for agent in self.agents}
    ...
    def _clausula_satisfecha(self, agent):
        resultado_leyes = (self.votos_favor > self.votos_contra).astype(int)
        for var_idx, deseo in self.clausulas_privadas[agent]:
    ...
    def _crear_observacion(self, agent):
        ...
        local_obs = np.concatenate([
            clausula,              # 10: que variables me importan
            self.presion_neta,     # 10: direccion de la demanda colectiva
            self.presion_magnitud, # 10: cuanto interes hay en cada variable
            paso_norm,             # 1
            satisfecho,            # 1
        ])
