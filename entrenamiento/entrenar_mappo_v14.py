...
class SplitMlpExtractor(nn.Module):
    def __init__(self, feature_dim, local_dim=52, global_dim=42, hidden_size=128):
        super().__init__()
...
def entrenar():
    ...
    print("=" * 60)
    print("  ENTRENAMIENTO MAPPO - V14 (mentira estrategica)")
    print("=" * 60)
    print(f"  Agentes: {NUM_AGENTES} | Variables: {NUM_VARIABLES}")
    print(f"  Timesteps: {TOTAL_TIMESTEPS:,}")
    print(f"  Arquitectura: MAPPO (actor {Entorno3SAT.LOCAL_OBS_DIM} / critico {Entorno3SAT.GLOBAL_STATE_DIM})")
    print(f"  p_inyeccion: {P_INYECCION}")
    print("=" * 60)

    env = Entorno3SAT(num_agentes=NUM_AGENTES, num_variables=NUM_VARIABLES,
                      p_inyeccion=P_INYECCION)
    ...
    checkpoint = CheckpointCallback(
        save_freq=100_000,
        save_path=MODEL_DIR,
        name_prefix="mappo_3sat_v14_mentira"
    )

    print("\nEntrenando...")
    model.learn(total_timesteps=TOTAL_TIMESTEPS, callback=checkpoint)

    ruta_final = os.path.join(MODEL_DIR, "mappo_3sat_v14_mentira_final")
