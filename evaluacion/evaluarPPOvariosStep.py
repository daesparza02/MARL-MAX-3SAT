...
def graficar_lineas_negociacion(historial_votos_pasos, titulo, num_agentes):
    try:
        ...
        umbral = num_agentes / 2
        plt.axhline(y=umbral, color='red', linestyle='--', linewidth=2, label=f'Mayoría ({umbral})')

        plt.title(f"{titulo} | NEGOCIACIÓN | Evolución Turno a Turno")
        plt.xlabel("Turno de Negociación (Pasos 1 al 15)")
        ...
        plt.tight_layout()
        print(f"   > Abriendo gráfica de LÍNEAS para: {titulo}...")
        plt.show()
    except Exception as e:
...
def ejecutar_partida(env_raw, model, caso_datos=None):
    ...
    while not terminado:
        ...
        historial_votos_pasos.append(env_raw.estado_votacion.copy())

        terminado = all(terms.values())

    votos_reales = [acciones_dict[agent] for agent in env_raw.possible_agents]
...
def evaluar():
    ...
    nombre_archivo = "ppo_3sat_v6_egoistaInformado_final.zip"
