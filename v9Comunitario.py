...
class Entorno3SAT(ParallelEnv):
    metadata = {"render_modes": ["human"], "name": "voto_3sat_v9_secuencial_comunitario"}
    ...
    def step(self, actions):
        ...
        delta = satisfechos_ahora - self._satisfechos_anterior

        self._satisfechos_anterior = satisfechos_ahora

        rewards = {}
        for agente in self.agents:
            rewards[agente] = float(delta)

        if terminado:
            # Bonus final: total de clausulas satisfechas
            clausulas_satisfechas = 0
            resultado_leyes = (self.resultados_decididos > 0.5).astype(int)
            for agent in self.agents:
                clausula = self.clausulas_privadas[agent]
                if any(resultado_leyes[var_idx_c] == deseo for var_idx_c, deseo in clausula):
                    clausulas_satisfechas += 1
            bonus = float(clausulas_satisfechas)
            for agente in self.agents:
                rewards[agente] = rewards[agente] + bonus
